...
class Review:
    def __init__(self, reviewLine):
        ...
        self.classification = reviewLine[:spacePos] == "1"  # Checks if this is a good or bad review
...
def import_data():
    ...
    with open(DATA_FILE_NAME, "r") as tf:  # Reading from data file
        allReviews = [Review(review) for review in tf.readlines()]  # Store every review as a class object
...
def import_custom_data():  # Use all simple reviews as training and two custom reviews as test data
    ...
    with open(DATA_FILE_NAME, "r") as tf:  # Reading from data file
        trainingData = [Review(review) for review in tf.readlines()]  # Store every review as a class object
...
for word in wordsFound.keys():  # Store number of good and bad reviews for each word as a fraction
    classNums = wordsFound[word]  # Number of good and bad reviews from this class

    if classNums[0] == 0 or numGood == 0:
        classNums[0] = (classNums[0] + kValue) / (numGood + numClasses * kValue)
    else:
        classNums[0] /= numGood
...
print("P(Positive | Review) = {}, P(Negative | Review) = {}".format(pReviewGivenGood, pReviewGivenBad))
